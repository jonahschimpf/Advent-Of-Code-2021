...
fn main() {
    ...
    for pos in 0..max_pos + 1 {
       ...
       let fuel_list_for_pos: Vec<i32> = get_fuel_list_for_pos(pos, max_pos, count, &mut memo);
...
fn get_fuel_list_for_pos(pos: i32, max_pos: i32, count: i32, memo: &mut HashMap<i32, i32>) -> Vec<i32> {
   let mut ret: Vec<i32> = vec![0; (max_pos + 1) as usize]; 
   for i in 0..max_pos + 1 {
        let value = if count > 0 { calculate_rate((pos - i).abs(), memo) * count } else { 0  };
        ret[i as usize] = value;
   }
   return ret;
}

fn calculate_rate(dist: i32, memo: &mut HashMap<i32, i32>) -> i32 {
    if dist == 0 {
        return 0;
    } else if memo.contains_key(&dist) {
        return *memo.get(&dist).unwrap();
    }
    let mut count = 0;
    for i in 1..dist + 1{
        count += i;
    }
    memo.insert(dist, count);
    return count;
}
