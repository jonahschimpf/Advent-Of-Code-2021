...
fn main() {
    ...
    let mut col_size = 0;
    let mut row_size = 0;
   let (vertz, hortz) = get_vertz_and_hortz(&lin_vec, &mut col_size, &mut row_size); 
...
fn get_vertz_and_hortz(contents: &Vec<String>, cols_size: &mut i32, row_size: &mut i32) -> (Vec<(Pair, Pair)>, Vec<(Pair, Pair)>) {
